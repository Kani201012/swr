...
import json
import datetime
import re  # <--- REQUIRED for the new Text Formatter

# --- 1. APP CONFIGURATION ---
st.set_page_config(
    page_title="Titan v30.5 | StopWebRent Edition", 
    layout="wide", 
    page_icon="⚡",
...
with st.sidebar:
    ...
    with st.expander("🧩 Section Manager", expanded=False):
        ...
        show_pricing = st.checkbox("Pricing Comparison Table", value=True) # --- NEW FEATURE ---
...
with tabs[0]:
    ...
    with c1:
        ...
        biz_email = st.text_input("Email (For Forms)", "hello@stopwebrent.com")
    with c2:
        prod_url = st.text_input("Website URL (Required for Sitemap)", "https://stopwebrent.com")
        ...
        seo_d = st.text_area("Meta Description (SEO)", "The Titan Engine is 10x faster and $0 monthly fees. Stop renting your website from Wix or Shopify. Pay once, own it forever.", height=100)
...
def format_text(text):
    ...
    # This regex finds double asterisks and wraps content in <strong> tags
...
def get_simple_icon(name):
    ...
    # 1. Tech & SaaS (Including StopWebRent Icons)
...
if show_pricing: home_content += gen_pricing_table() # --- NEW ADDITION ---
